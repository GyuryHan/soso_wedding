					...
					function() {
						...
						//댓글 수정 View
							...
							.on(
									"click",
									function() {
										var repleForm = $(".comment-box").length;
											var go = $(this).attr("data-rno");
											$(".reple"+go).load("/board/replyUpdateView?bno=${read.bno}"
													+ "&page=${scri.page}"
													+ "&perPageNum=${scri.perPageNum}"
...
$(document).ready(function(){
   $(".category2").change(function(){
      ...
      switch (category){
         case "101" :
            $(".areacontain").load("/company/goods/details/weddinghalldetail");
             break;
       }
   });
	...
	<div class="boardwrap">
		...
		<div class="boardwrap3">
			...
			<div class="row" id="down-btn">
				<c:if
					test="${mp_member.userId eq mp_board.writer && mp_member.userId != null}">
               ...
               <input type="hidden" id="keyword" name="keyword" value="${scri.keyword}">
					...
					<c:forEach items="${replyList}" var="replyList" varStatus="status">
						<div class="comment-box">
							...
							<span class="reple${replyList.rno}">
